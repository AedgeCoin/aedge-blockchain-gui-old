...
import { mojo_to_aedge } from '../../util/aedge';
import {
  calculatePoolReward,
...
export default function Block() {
  ...
  const poolReward = mojo_to_aedge(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_aedge(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const aedgeFees = blockRecord.fees
    ? mojo_to_aedge(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://alltheblocks.net/aedgecoin/block/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://alltheblocks.net/aedgecoin/block/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: aedgeFees ? `${aedgeFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
