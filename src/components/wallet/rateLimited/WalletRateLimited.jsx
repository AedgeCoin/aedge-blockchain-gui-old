...
import { AlertDialog, Card, Flex } from '@aedge/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const aedgeper_input = ip_parsed.limit;
    ...
    const aedgeper_value = Number.parseInt(Number(aedgeper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (aedge per interval):{' '}
                  {mojo_to_aedge_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = aedge_to_mojo(fee_input.value);
