...
import { AlertDialog, Card, Flex, Loading, Dropzone } from '@aedge/core';
import {
...
import { mojo_to_aedge_string } from '../../../util/aedge';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_aedge_string(props.balance)} TAEC
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const CreateAttest = (props) => {
  ...
  function createAttestPacket() {
    ...
    if (address.substring(0, 12) === 'aedge_addr://') {
...
const CashoutCard = (props) => {
  ...
  function cashout() {
    ...
    if (puzzlehash.slice(0, 12) === 'aedge_addr://') {
