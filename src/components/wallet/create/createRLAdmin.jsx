...
import { AlertDialog } from '@aedge/core';
import {
  Typography,
  Button,
  Box,
...
import { aedge_to_mojo } from '../../../util/aedge';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      aedgeper_input.value === '' ||
      Number(aedgeper_input.value) === 0 ||
      !Number(aedgeper_input.value) ||
      isNaN(Number(aedgeper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const aedgeper = aedge_to_mojo(aedgeper_input.value);
    const aedgeper_value = Number.parseInt(Number(aedgeper));
    const userpubkey = userpubkey_input.value;
    const amount = aedge_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = aedge_to_mojo(fee_input.value);
