...
const getAedgeVersion = () => {
  let version = null;
  const exePath = getExecutablePath('aedge');
  // first see if we can get a aedge exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or aedge is on the path
    try {
      version = child_process
