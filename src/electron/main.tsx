...
import aedgeEnvironment from '../util/aedgeEnvironment';
import aedgeConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!aedgeEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    aedgeConfig.loadConfig('mainnet');
    ...
    const createWindow = async () => {
      ...
      if (!aedgeConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !aedgeConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (aedgeConfig.manageDaemonLifetime()) {
        aedgeEnvironment.startAedgeDaemon();
      }
    };
